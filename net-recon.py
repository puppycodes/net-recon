        ...
        for session in sessions:
            ...
            for packet in sessions[session]:
                ...
                if packet.getlayer(Ether) and packet[Ether].dst == "01:80:c2:00:00:0e":
                    ...
                    if hostname not in self.keys['hosts'].keys():
                        self.keys['hosts'].update({hostname:{'mac': mac, 'fingerprints': system_description, 'management_ipv4': mgt_ipv4, 'system_name': system_name, 'tr41-location-id': address, 'protocol': 'LLDP'}})
